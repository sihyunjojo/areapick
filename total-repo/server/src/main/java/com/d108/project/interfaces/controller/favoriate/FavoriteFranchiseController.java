...
public class FavoriteFranchiseController implements FavoriteFranchiseApi {
    ...
    @Override
    public ResponseEntity<Object> createFavoriteFranchise(Member member, Long favoriteFranchiseId) {
        favoriteFranchiseService.createFavoriteFranchises(member.getId(), favoriteFranchiseId);
        return ResponseEntity.ok().build();
    }
}
