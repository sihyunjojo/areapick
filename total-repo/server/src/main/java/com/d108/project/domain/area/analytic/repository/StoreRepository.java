...
public interface StoreRepository extends JpaRepository<Store, Long> {
    @Query("SELECT s.similarBusinessCount FROM Store s WHERE s.area.id = :areaId and s.serviceType.serviceCode = :serviceCode")
    List<Integer> getNumberOfSimilarStores(@Param("areaId") Long areaId, @Param("serviceCode") String serviceCode);
    // 업종 정보 조회
    @Query("SELECT s.storeCount, s.openingRate, s.closingRate FROM Store s WHERE s.area.id = :areaId and s.serviceType.serviceCode = :serviceCode")
