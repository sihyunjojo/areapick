...
function createPost(payload, success, fail) {
    local.post(`/api/v1/posts`, payload).then(success).catch(fail);
}
...
function getPostListByBoard(boardId, page, size, success, fail) {
    ...
    local
        .get(`/api/v1/posts/${boardId}/${page}/${size}`)
        .then((response) => {
...
function updatePost(postId, payload, success, fail) {
    local.put(`/api/v1/posts/${postId}`, payload).then(success).catch(fail);
}

// 게시글 삭제 API
function deletePost(postId, success, fail) {
    // deletePostAPI -> deletePost
    local.delete(`/api/v1/posts/${postId}`).then(success).catch(fail);
}

// 댓글 등록 API
function createReply(postId, payload, success, fail) {
    // submitReplyAPI -> createReply
    local.post(`/api/v1/posts/${postId}/replies`, payload).then(success).catch(fail);
}

// 댓글 목록 조회 API
function getReplyList(postId, replyId, success, fail) {
    // getRepliesAPI -> getReplyList
    local.get(`/api/v1/posts/${postId}/replies/${replyId}`).then(success).catch(fail);
}

// 댓글 수정 API
function updateReply(postId, replyId, payload, success, fail) {
    local.put(`/api/v1/posts/${postId}/replies/${replyId}`, payload).then(success).catch(fail);
}

// 댓글 삭제 API
function deleteReply(postId, replyId, success, fail) {
    console.log('삭제할 댓글 API 호출 - Post ID:', postId, 'Reply ID:', replyId); // 확인용 로그
    local.delete(`/api/v1/posts/${postId}/replies/${replyId}`).then(success).catch(fail);
}
